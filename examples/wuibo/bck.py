...
class RandBot(interface.Bot):
    ...
    def __init__(self,init_state):
        ...
        for pos in self.lighthouses:
            """turnos a faro"""
            light_dict[pos] = utils.lighthouse(pos,self.maxy,self.maxx,flood_dist(pos))
            """energia por turno"""
            for y in xrange(pos[1]-self.E_DIST+1,pos[1]+self.E_DIST):
                for x in xrange(por[0]-self.E_DIST+1,pos[0]+self.E_DIST):
        ...
        for i in xrange(tri_total):
            ...
            if points > 0
                ...
                energy = (perimeter + E_TURN + perimeter/E_FRAC)*E_LOSS
    ...
    def play(self, state):
        ...
        for pos,lh in lighthouses.iteritems():
            if lh["owner"] >= 0 and lh["owner"] != self.player_num:
                self.lighthouses[pos].sum_ocupied()
    ...
    def get_objetive(self,lhs,cx,cy):
        ...
        for tr in self.tri:
            propios = [] """los propios, con llave o si llave"""
            otros = [] """el resto"""
            ...
            if len(cnd) == 0:
                ...
                if cnd[0][1] == 1:
                    ...
                    df0 = self.light_dict[otros[0]].get_dist((cx,cy))
                    df1 = self.light_dict[otros[1]].get_dist((cx,cy))
                    dotros = self.lifht_dict[otros[0]].get_dist(otros[1])
                    dc0 = self.light_dict[otros[0]].get_dist(cnd[0][0])
                    dc1 = self.ligth_dict[otros[1]].get_dist(cnd[0][0])
                    dt0c = df0 + dc0 + dc1 + 6
                ...
                # controlo 2
                if cnd[0][0] in lhs[cnd[1][0]]["connections"]:
                    # están conectados
                    if cnd[0][1] == 1 and cnd[1][1] == 1:
                        ...
                        if to0 < to1:
                            ...
                            # conectar al de menor energia primero
                        ...
                        df2 = self.ligth_dict[otros[0]].get_dist((cx,cy))
                        ...
                        dt2120 = df2 + d12 + d12 + d02 + 6
                        dt2102 = df2 + d12 + d01 + d02 + 6
                ...
                # controlo 3
                if not(cnd[0][0] in lhs[cnd[1][0]]["connections"] and cnd[1][0] in lhs[cnd[2][0]]["connections"] and cnd[2][0] in lhs[cnd[0][0]]["connections"]):
                    # falta alguna conexion
                    if cnd[0][0] in lhs[cnd[2][0]]["connections"] and cnd[1][0] in lhs[cnd[2][0]]["connections"]:
                        # Falta la 0-1
                        ...
                        if cnd[0][1] == 1 and cnd[1][1] == 1:
                            ...
                            if d0 < d1:
                                ...
                                if lhs[cnd[0][0]]["energy"] <= lhs[cnd[1][0]]["energy"]:
                                    ...
                                    n_next = cnd[1][0]
                                    n_turn = d1 + dt + 3
                    elif cnd[1][0] in lhs[cnd[0][0]]["connections"] and cnd[2][0] in lhs[cnd[0][0]]["connections"]:
                        # Falta la 1-2
                        d1 = self.light_dict[cnd[1][0]].get_dist((cx,cy))
                        ...
                        if cnd[1][1] ==1 and cnd[2][1] == 1:
                            ...
                            if d1 < d2:
                                ...
                                if lhs[cnd[1][0]]["energy"] <= lhs[cnd[2][0]]["energy"]:
                                    ...
                                    n_next = cnd[2][0]
                                    n_turn = d2 + dt + 3
                    elif cnd[0][0] in lhs[cnd[1][0]]["connections"] and cnd[2][0] in lhs[cnd[1][0]]["connections"]:
                        # Falta la 0-2
                        d0 = self.light_dict[cnd[0][0]].get_dist((cx,cy))
                        d2 = self.light_dict[cnd[2][0]].get_dist((cx,cy))
                        ...
                        if cnd[0][1] == 1 and cnd[2][1] == 1:
                            ...
                            if d0 < d2:
                                ...
                                if lhs[cnd[0][0]]["energy"] <= lhs[cnd[2][0]]["energy"]:
                                    ...
                                    n_next = cnd[2][0]
                                    n_turn = d2 + dt + 3
                    elif cnd[0][0] in lhs[cnd[2][0]]["connections"]:
                        # Faltan 0-1 y 1-2
                        df0 = self.light_dict[cnd[0][0]].get_dist((cx,cy))
                        df1 = self.light_dict[cnd[1][0]].get_dist((cx,cy))
                        ...
                        if cnd[0][1] == 1 and cnd[2][1] == 1:
                            ...
                            if cnd[1][1] == 1:
                                ...
                                n_turn = min(d210,d012,d1021,d1012,d1201,d1210)
                                if n_turn != d2 and n_turn != d0:
                                    ...
                                    if lhs[cnd[1][0]]["energy"] <= lhs[cnd[0][0]]["energy"] and lhs[cnd[cnd[1][0]]["energy"] <= lhs[cnd[2][0]]["energy"]:
                                        n_next = cnd[1][0]
                                    elif lhs[cnd[0][0]]["energy"] <= lhs[cnd[2][0]]["energy"]:
                                        n_next = cnd[0][0]
                                    else:
                                        n_next = cnd[2][0]
                    elif cnd[0][0] in lhs[cnd[1][0]]["connections"]:
                        # Faltan 1-2 y 0-2
                        df0 = self.light_dict[cnd[0][0]].get_dist((cx,cy))
                        df1 = self.light_dict[cnd[1][0]].get_dist((cx,cy))
                        df2 = self.light_dict[cnd[2][0]].get_dist((cx,cy))
                            ...
                            if cnd[2][1] == 1:
                                ...
                                if n_turn != d0 and n_turn != d1:
                                    ...
                                    if lhs[cnd[2][0]]["energy"] <= lhs[cnd[0][0]]["energy"] and lhs[cnd[2][0]]["energy"] <= lhs[cnd[1][0]]["energy"]:
                                        ...
                                        n_next = cnd[1][0]
                    elif cnd[1][0] in lhs[cnd[2][0]]["connections"]:
                        # Faltan 0-1 y 0-2
                        df0 = self.light_dict[cnd[0][0]].get_dist((cx,cy))
                        df1 = self.light_dict[cnd[1][0]].get_dist((cx,cy))
                        df2 = self.light_dict[cnd[2][0]].get_dist((cx,cy))
                        ...
                        dfc = self.light_dict[cnd[1][0]].get_dict(cnd[2][0])
                        if cnd[1][1] == 1 and cnd[2][1] == 1:
                            # tenemos las dos llaves
                            n_next = cnd[0][0]
                            n_turn = df0 + 3
                        ...
                        if cnd[0][1] == 1 and cnd[1][1] == 1 and cnd[2][1] == 1:
                            ...
                            if n_turn != d1 and n_turn != d2:
                                ...
                                # triple empate
                                if lhs[cnd[0][0]]["energy"] <= lhs[cnd[1][0]]["energy"] and lhs[cnd[0][0]]["energy"] <= lhs[cnd[2][0]]["enery"]:
                                    n_next = cnd[0][0]
                                    if d01 < d20:
                                        n_connect = cnd[2][0]
                                    else:
            ...
            if to == -1:
                ...
                if nxt_dist*E_LOSS >= light_ener:
                    ener = tr.get_energy()
                else:
                    ener = tr.get_energy + light_ener - (nxt_dist*E_LOSS)
    ...
    def check_tri_cross(self,light,tri):
        ...
        return (check_croses(light,t_lh[0],t_lh[1]) or check_croses(light,lh[1],lh[2]) or check_croses(light,lh[2],lh[0]))
